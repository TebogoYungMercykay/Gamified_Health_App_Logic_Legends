...
export default function Example() {
	...
	let selectedDayEvents = Events.filter((meeting) =>
						...
						{days.map((day, dayIdx) => (
							<div key = {day.toString()} className = {classNames( dayIdx === 0 && colStartClasses[getDay(day)], 'py-1.5' )}>
								...
								<div className="w-1 h-1 mx-auto mt-1">
									{Events.some((meeting) => isSameDay(parseISO(meeting.startDatetime), day) ) && ( <div className="w-1 h-1 rounded-full bg-sky-500"></div> )}
								</div>
							</div>
						))}
					</div>
